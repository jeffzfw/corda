...
interface DriverDSLExposedInterface {
    /**
     * Starts a [net.corda.node.internal.Node] in a separate process. This is a convenience method which converts
     * the provided name to an [X500Name] automatically.
     *
     * @param providedName Optional name of the node, which will be its legal name in [Party].
    ...
    fun startNode(providedName: String,
                  ...
                  customOverrides: Map<String, Any?> = emptyMap()) = startNode(X500Name(providedName),
            advertisedServices, rpcUsers, verifierType, customOverrides)
